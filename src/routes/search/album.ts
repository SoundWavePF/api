...
albumRouter.get('/:albumId', async (req, res) => {
    ...
    try {
        const albumsOnDb: AlbumResult = await db.album.findOne({
            attributes: { exclude: ['artistId', 'genreId'] },
            where: { id: albumId },
            include: [{ model: db.artist, attributes: { exclude: ['userId'] } }, db.genre, { model: db.song, attributes: { exclude: ['artist_id_reference', 'genre_id_reference', 'album_id_reference', 'albumId', 'artistId', 'genreId', 'userId'] }, include: [{ model: db.album, attributes: ['id','name'] }, { model: db.artist, attributes: ['id','name'] }] }]
        })
        return res.send(albumsOnDb)
    } catch (e: any) {
        return res.send({ message: e.message })
